...
public class H264EncodeThread extends Thread {
    ...
    private Demo7Activity activity;
    ...
    public H264EncodeThread(Demo7Activity activity, int width, int height, int framerate, int biterate) {
    ...
    public void run() {
        super.run();
        try {
            out264File = new File(activity.getExternalFilesDir(Environment.DIRECTORY_MOVIES),"ca2.h264");
            ...
            while (isEncode || !inOutFinish) {
                if (isEncode) {
                    //获取当前的yuv数据
                    if (!activity.getYUVQueue().isEmpty()) {
                        ...
                        if (inputIndex != -1) {
                            ...
                            //从yuv队列中取出数据
                            yuvData = activity.getYUVQueue().poll();
                            //摄像头默认是横着的，需要旋转90度
...
//                            NV21ToNV12(revolveData, yuv420sp, height, width);
                            ...
                            if (yuvData.length < 0) {
                                ...
                                encodeCodec.queueInputBuffer(inputIndex, 0, yuvData.length, pts, 0);
                                generateIndex += 1;
                            }
                        } else {
    ...
    private void capture(byte[] data) {
        File img = new File(activity.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "sdf.jpg");
        try {
            img.createNewFile();
